...
class TweetTableViewController: UITableViewController, UITextFieldDelegate
{
    ...
    // recentQuery to get and set queries in history in userdefault
    //private var recentQuery = RecentQuery()
    //public var recentQuery = RecentQuery()
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let DetailViewController = destinationViewController as? DetailTableViewController, let currentCell = sender as? TweetTableViewCell {
            //Though we only have one segue, we still use identifier
            if segue.identifier == "detail" {
                DetailViewController.tweet = currentCell.tweet
            }
        }
    }
    
    //Array of array for tweet
